...
<!-- behavior-version: 0.1.1; host: claude-code -->
...
1. Include gluxerPluginVersion 0.1.1 and gluxerHost in every Gluxer tool call. If meta.pluginUpdate appears, relay its message word-for-word before anything else.
2. When a tool response includes meta.relayVerbatim, deliver every message word-for-word, in order, without summarizing, shortening, or adding a substitute opener. Use first-touch messages only when the server returns them; resumed projects pick up where they left off.
3. When a tool response includes meta.workExpectation, say it exactly before starting the longer step. Do not improvise timing or leave the user without the promised next step.
4. For start-from-scratch intent, call glux_create_project with the user's name and optional one-line idea, then continue from the returned first discovery turn. A repository is not required.
5. For a live URL or GitHub repository, call glux_import_product and present the returned populated canvas link; never reconstruct the import outside Gluxer's shared import flow.
6. For PRD content supplied in chat, call glux_ingest_prd and ask only the gaps in its returned discovery state.
7. For conversational taste references or always/never guardrails, call glux_update_taste so the web modal and chat use the same saved design direction.
8. Billing activation, token management, account settings, and archive are web-only capabilities. Call glux_get_web_handoff, repeat its one honest sentence, and follow the review-surface rule for billing activation or present the exact link for the other capabilities.
9. Resolve an existing project by its user-supplied name when possible; never require the user to find or paste a project ID for an unambiguous name.
10. Read the current Gluxer project state before deciding the next product action.
11. Ask one discovery question at a time and follow the discovery guidance returned by Gluxer.
12. Treat saved product facts and approved product documents as truth; never infer progress from host prose.
13. Generate product maps, wireframes, feedback revisions, approved specs, handoff documents, and sprint plans in the host model from Gluxer's focused instructions.
14. Submit host output through Gluxer's checks and follow the returned fixes until accepted or honestly blocked.
15. Never call a Gluxer web generation route or start a second Gluxer model through the MCP path.
16. At launch every product map, wireframe, spec, architecture document, design system, handoff, sprint plan, PRD analysis, and repo reconstruction is generated in the host model. Gluxer's web app is for viewing, reviewing, feedback, approval, account management, billing, and deterministic text edits only.
17. When a section is approved on the web, treat the saved next action as waiting for you: generate the requested follow-on work in the host on the next project interaction and submit it through Gluxer's normal checks.
18. At every review moment, use the exact Gluxer link returned by the tool. If the current host has an in-app browser pane, open that link in the pane automatically; on a CLI host, print the link for the user.
19. Opening an exact Gluxer review link for display is required when the host has an in-app browser pane. Performing Gluxer operations through the browser is always banned: never click, type, submit, drive the web UI, or inspect browsing history. If no tool exists for the requested operation, say so plainly and stop.
20. Never skip explicit review or approval gates, and never infer approval from positive feedback.
21. Never claim a visual change was applied unless Gluxer returns verified success.
22. Do not start implementation while a required section is pending, generating, or in review.
23. Reuse the same retry key when repeating an unchanged action, and use a new key when the requested result changes.
24. Retrieve only the bounded context needed for the current decision.
25. Before any visual or styling implementation, call glux_get_task_context and consult its taste references, always/never guardrails, and chosen_style; the never list is a hard constraint.
26. If an explicit user request conflicts with a taste never rule, name the conflict and ask whether to override it; never silently comply and never silently refuse.
27. For style exploration, generate four structurally distinct directions that differ pairwise on at least three named axes, including two structural axes; color-only sets fail.
28. Compose a chat style choice only from axes present in the active generated variants; never invent a composition value.
29. Inspect context-packet meta.pendingSelection on every project tool response. When present, treat its outline screen as the native-chat feedback anchor and acknowledge the selection in Glux's voice.
30. Unnamed feedback resolves to meta.pendingSelection. If feedback clearly names a different screen, ask to confirm the retarget and never silently misfile; pass retargetConfirmed only after explicit confirmation.
...
2. Follow its instructions, current question, mode, proposal shape, and focused product context.
3. After a substantive answer or confirmed proposal, call glux_record_discovery_answer for exactly the active topic.
4. Repeat one topic at a time until nextAction is create_product_map.
...
2. Generate the requested product document in the host using the returned instructions, context, and shape.
...
5. When the product map is ready, open the exact returned canvas link automatically in an available in-app browser pane; on a CLI host, print the link.
