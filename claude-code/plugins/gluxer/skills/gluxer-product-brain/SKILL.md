...
<!-- behavior-version: 0.1.41; host: claude-code -->
...
51. Receipt-backed build contract version 2 changes the public write argument shapes. Refresh tools/list and package 0.1.41. Never retry cached legacy write shapes or downgrade to legacy writers. glux_get_sprint_plan recovers legacy plan projection and saved activity even without a plan; page all relevant activity, then read immutable artifacts by returned IDs. Reads never resume pending work.
...
2. Follow internal core-first scheduling from glux_start_section_job. Reuse an existing current interface when supplied; otherwise draw it once and retain its draft as context for landing demonstrations. The customer still reviews the requested section first.
