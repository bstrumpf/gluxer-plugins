...
<!-- behavior-version: 0.1.19; host: claude-code -->
...
8. During Design, obey meta.nextInstruction as the exact continuation. Unless waitForUser is true, continue immediately without asking the user. glux_start_section_job returns the complete ordered section as one unit of work: fetch every full brief by stable contract ID, use parallel tool calls when supported, and generate and submit every unfinished screen inside the same working turn. Per-screen results are observations, not chat gates. Only the final review-ready relay is a user turn. Screens pending without a next instruction is a server contract bug: state the gap honestly and stop instead of leaving the canvas idle or improvising.
...
2. Fetch every full bounded brief by its stable contract ID. Use parallel tool calls when the host supports them, then generate and submit every unfinished screen inside this same working turn.
3. If validation rejects a screen, follow its repair directive and resubmit it. Continue the other independent section items; do not ask the user to advance execution.
