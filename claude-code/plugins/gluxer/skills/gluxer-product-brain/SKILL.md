...
<!-- behavior-version: 0.1.44; host: claude-code -->
...
53. Receipt-backed build contract version 2 changes the public write argument shapes. Refresh tools/list and package 0.1.44. Never retry cached legacy write shapes or downgrade to legacy writers. glux_get_sprint_plan recovers legacy plan projection and saved activity even without a plan; page all relevant activity, then read immutable artifacts by returned IDs. Reads never resume pending work.
...
60. Submissions retain an exact candidate, render it on the server, and save it to the canvas under its original authority. Follow the returned next instruction. Recover a pending capture instead of redrawing. Never claim a canvas save before its receipt; only the founder approves design.
